...
public class Ball {
	...
	public Ball(World inWorld, Vector2 SpawnLoc)
	{
		curWorld = inWorld;
		

		float SpawnDirection = -90;
		SpawnDirection += -45 + Math.random() * 90;
		System.out.print("Spawn angle " + SpawnDirection);
		...
		Velocity.x = (float)Math.cos(Math.toRadians(SpawnDirection)) * Speed;
		Velocity.y = (float)Math.sin(Math.toRadians(SpawnDirection)) * Speed;		
	}
	...
	public void Update(float DeltaTime)
	{
		...
		else if (Location.y > 899 - BALLSIZE)
		{
			Velocity.y *= -1;
			Location.y = 899 - BALLSIZE;			
		}
